...
namespace detail {
	...
	struct underlying_type_safe {
		using type = std::underlying_type_t<T>;
	};

	template<typename T>
	struct underlying_type_safe<T, false> {
		using type = T;
	};

	template<typename T>
	using underlying_type_safe_t = typename detail::underlying_type_safe<T, std::is_enum_v<T>>::type;

	template <typename To, typename From>
	[[nodiscard]] constexpr To bit_int_cast(const From value) noexcept
	{
		using IntegerFrom = underlying_type_safe_t<From>;
		static_assert(std::is_integral_v<IntegerFrom> && std::is_integral_v<To>);

		const auto value_unsigned = static_cast<std::make_unsigned_t<IntegerFrom>>(value);
		const auto result_unsigned = static_cast<std::make_unsigned_t<To>>(value_unsigned);
		return static_cast<To>(result_unsigned);
	}
}
...
namespace detail {
	...
	template <>
	class EnumReflection<NtStatus>
	{
		using EnumType = NtStatus;
	...
	public:
		static constexpr std::string_view enum_name(EnumType v) noexcept {
			const auto it = std::find_if(_items.begin(), _items.end(), [v](auto&& item) {
				return item.second == ::detail::bit_int_cast<ValueType>(v);
			});
	...
	private:
		static constexpr std::array<std::pair<std::string_view, ValueType>, 330> _items{{
			{ "Success", 0 },
{ "Wait0", 0 },
{ "Wait1", 1 },
{ "Wait2", 2 },

...
// filtering) sits next to a big grid of staged video cards (MediaItemWidget, same as the main grid).
...
// Catalog until "Import" runs: every staged video that's been labeled gets imported and removed from
...
public:
	...
	struct Callbacks
	{
		...
		// (<collection>/<source base name>) - checked right after addMediaItemsRequested for each video in its
		...
		// Called once per successful Import, with every imported video's extra-label picks (may be empty).
		// Mirrors markBestRequested above: the videos have already been added via addMediaItemsRequested by then.
		std::function<void(const QList<ExtraLabelAssignment>& assignments)> assignExtraLabelsRequested;
		// Called once at the end of an Import that imported at least one video, after the Best/extra-label flush
		// above. addMediaItemsRequested may refresh the host view mid-Import (it imports folder-by-folder), but the
		// post-import label flush has no refresh of its own - so without this, the host shows each video's

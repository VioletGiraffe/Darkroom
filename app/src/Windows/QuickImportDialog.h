...
// ============================================================================
// QuickImportDialog - stage new video files, label them, then ingest everything labeled in one "Import" step.
//
// Mirrors the main window's own label model: a small label-list panel (like LabelSidebar, minus
// filtering) sits next to a big grid of staged video cards (VideoItemWidget, same as the main grid).
// Dragging a label from the list onto a staged card tags it, exactly like dragging a label onto a video
// card in the main window; the star button toggles Best the same way too. Nothing is written to the
// Catalog until "Import" runs: every staged video that's been labeled gets ingested and removed from
...
// stands in for it. Unlabeled videos are left alone either way. Files can also be dropped straight onto
...
public:
	...
	// One staged video's extra-label picks, resolved against the collection it landed in (needed to derive
	// its frame folder, the same way markBestRequested's per-collection grouping does for Best). Identified by
	// VideoId, not path: relocation (Move) deletes the source from its staged path before this is applied, so
	...
	struct Callbacks
	{
		// Every ordinary label, for the label-list panel. A staged video's first-dropped label also
		// decides which one of these folders it's ingested into - see runImport() in the .cpp.
		std::function<QList<LabelOption>()> getLabelOptions;
		// Adds the given video files to the named collection. stagedPreviewDirs maps each staged video's VideoId
		// to the temp dir whose preview/ holds the frames already extracted for its staging card, so ingestion
		...
		std::function<void(const QString& collectionName, const QStringList& videoPaths,
			const QHash<VideoId, QString>& stagedPreviewDirs)> addVideosRequested;
		...
		// True iff the video is now tracked by the Catalog at the frame folder this import derives for it
		// (<collection>/<source base name>) - checked right after addVideosRequested for each video in its
		...
		// on a name+size collision the id is already tracked elsewhere and ingestion refused the staged copy -
		// a mere "is tracked" check would misreport that as a success, unstaging the entry and silently
		// dropping its pending labels. By VideoId, not path: Move relocation has already deleted the source
		// from its staged path by now.
		std::function<bool(const VideoId&, const QString& collectionName)> isVideoTrackedInCollection;
		// Called once per successful Import, with the videos flagged "Best" keyed by the collection they were
		// added to (may be empty). The flagged videos have already been added - and are therefore tracked -
		// via addVideosRequested. By VideoId for the same reason as ExtraLabelAssignment above.
		std::function<void(const QHash<QString, QList<VideoId>>& bestVideosByCollection)> markBestRequested;
		// Called once per successful Import, with every ingested video's extra-label picks (may be empty).
		// Mirrors markBestRequested above: the videos have already been added via addVideosRequested by then.
		std::function<void(const QList<ExtraLabelAssignment>& assignments)> assignExtraLabelsRequested;
		// Called once at the end of an Import that ingested at least one video, after the Best/extra-label flush
		// above. addVideosRequested may refresh the host view mid-Import (it ingests folder-by-folder), but the
		// post-ingest label flush has no refresh of its own - so without this, the host shows each video's
	...
	// nothing's been persisted yet); see "Source video relocation row" in the .cpp.
...
private:
	...
	void stageVideos(const QStringList& paths);
	// Deletes a staged entry's temp preview dir and removes its card; used by "Remove from staging" and
	// once an entry has been successfully ingested by runImport().
	void unstage(const VideoId& id);
	// Re-derives a staged card's label dots from its pendingLabelIds.
	void updateCardLabelDots(const VideoId& id);
	// The staged videos a card-targeted action applies to: the whole staged selection when `id` is part of a
	// multi-selection, otherwise just `id`. Mirrors MainWindow::effectiveSelection for the staged grid.
	[[nodiscard]] QList<VideoId> stagedSelection(const VideoId& id) const;
	void showStagedCardContextMenu(const VideoId& id, const QPoint& globalPos);
	// Every staged entry whose pendingLabelIds isn't empty: grouped by its first label and ingested via
	// addVideosRequested, then markBestRequested/assignExtraLabelsRequested for whatever landed.
	void runImport();
	[[nodiscard]] const LabelOption* findLabelOption(const QString& id) const;

private:
	// Per-staged-video state, accumulated by dragging labels from m_labelList onto the card in m_stagedGrid
	// and toggling its Best star; applied (and removed from here) only once "Import" runs successfully. The
	// first id in pendingLabelIds resolves the video's destination folder at Import time (see runImport) -
	...
	struct StagedEntry
	{
		...
		QListWidgetItem* item = nullptr;  // the grid item carrying this entry's VideoItemWidget card
	};

	Callbacks m_callbacks;
	QList<LabelOption> m_labelOptions;  // cached result of the last getLabelOptions() call
	QHash<VideoId, StagedEntry> m_staged;
	...
	// Source-video relocation controls - see "performRelocation" in QuickImportDialog.cpp.

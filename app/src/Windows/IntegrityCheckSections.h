...
// is skipped by the other. onClosed lets the section re-evaluate which blanket buttons still apply. Rows are owned
// by IntegrityCheckSections::m_rows, which the dialog keeps alive as long as itself - so every handler capturing a
// ResolvableRow* dies no later than the row it points at.
...
private:
	...
	struct VideoRow        { ResolvableRow* row; MediaId id; bool canReimport; bool canRegenerate; };
...
inline void ResolvableRow::close(const QString& status)
{
...
inline void IntegrityCheckSections::buildVideoIssues(const CatalogIntegrity::IntegrityReport& report)
{
	...
	const auto refreshBlanket = [this, reimportAllButton, regenerateAllButton, removeAllButton] {
	...
	for (const CatalogIntegrity::MediaIssue& issue : report.issues)
	{
		...
		wireSkip(skipButton, row);
		m_videoRows.push_back({ row, id, canReimport, canRegenerate });
	}
...
inline void IntegrityCheckSections::buildMissingPhotos(const CatalogIntegrity::IntegrityReport& report)
{
	...
	QObject::connect(locateAllButton, &QPushButton::clicked, locateAllButton, [this, refreshBlanket] {
		const QString dir = QFileDialog::getExistingDirectory(m_dialog, QObject::tr("Select folder to search for missing photos"));
		if (dir.isEmpty())
			return;
		...
		for (MissingPhotoRow& m : m_missingPhotoRows)
		{
			...
			if (m_callbacks.locatePhotoRequested(m.id, found.value())) { m.row->close(QObject::tr("Relocated to %1").arg(found.value())); ++relocated; }
			else ++failed;
		}
		refreshBlanket();

		QStringList msg{ QObject::tr("Relocated %1 photo(s).").arg(relocated) };
		if (unmatched)
			msg << QObject::tr("%1 had no matching file in that folder.").arg(unmatched);
		if (failed)
			msg << QObject::tr("%1 could not be relocated - the identity is already tracked as a different item.").arg(failed);
...
{
	QFrame* frame = new QFrame(m_content);
...
inline void IntegrityCheckSections::wireSkip(QPushButton* skipButton, ResolvableRow* row)
{
	QObject::connect(skipButton, &QPushButton::clicked, skipButton, [row] {
		row->close(QObject::tr("%1  (skipped)").arg(row->statusLabel->text()));
	});
}

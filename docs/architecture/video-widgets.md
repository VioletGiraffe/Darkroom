# Video card & thumbnail widgets

[← Back to architecture index](../../ARCHITECTURE.md)

## `VideoItemWidget` (`src/UiComponents/VideoItemWidget.h/.cpp`)

Card used in the MainWindow grid. Constructor takes the video's **`VideoId`** (see
[data-model.md](data-model.md)), plus preview paths/label/cell size/Best state/callbacks — no folder path;
the card has no use for one (label ops are `VideoId`-anchored, and the one remaining drag interaction is a
...
`videoId()` exposes the card's stable identity — carried so label ops address the video directly rather
...
left→right: the **Best-star toggle**, the **label dots**, then the **video name** (right-aligned, elided).
The card itself (object name `videoCard`) now owns the visual frame — a rounded border + hover highlight
(`Theme::Accent`/`AccentBg`), styled by `#videoCard` in the central `Style.cpp` sheet (not a per-instance
...
fall through) that paints small colored dots, **one per label the video carries, including `Best`** (Best is a
...
thumbnail's **top-right** corner (uncontested since the dots moved to the footer). `MainWindow::refreshVideoGrid`
sets it from whether the video is split yet — see [ingestion.md](ingestion.md) for the on-demand-split design
...
`VideoItemWidget` draws the card frame around thumbnail + footer instead (see "Card layout & frame" above).
...
**composite constructor** (used by `VideoItemWidget`'s grid cards) sets no drag payload at all and has no
way to acquire one — composite thumbnails are never drag sources. So `VideoItemWidget` and
...
while plain wheel falls through so the view scrolls; `VideoItemWidget` forwards it. **Owners apply the same
shape**: bound the new size, persist it to `QSettings`, and start a single-shot debounce timer that rebuilds
once the wheel settles — `MainWindow::zoomCards` (card height) → `refreshVideoGrid()`, and

...
`QListWidget` grid of `VideoItemWidget` cards, in a horizontal `QSplitter`. The card set is the sidebar's
...
`m_nameFilter` (toolbar `QLineEdit`, left of the view controls): a video-name substring filter ANDed with
...
*does* drive which cards get built, in `refreshVideoGrid`.) An earlier implementation post-filtered via
`folders.removeIf(...)` inside `refreshVideoGrid` itself, which meant every keystroke triggered a full grid
...
adjusts any option. `MainWindow` connects that to `resortVideoGrid()` and reads the state back through the
...
- `refreshVideoGrid()` — clears and rebuilds `m_videoGrid`, one `GridItem` (a `QListWidgetItem` carrying the
  card's `VideoId`) per shown video. Reads `cardImageHeight()` and `previewFrameCount` each rebuild, builds
  the card set from `Catalog::allVideos()` filtered by the sidebar's **label** filter
  (`Catalog::videosForLabel`), then ends with `applyNameFilter()` so the active name filter is honoured on the
...
- `applyNameFilter()` — hides cards not matching the name box (`setHidden`, deselecting any it hides) and
  ...
  cards.
- `showVideoContextMenu()` — multi-select-aware right-click menu. Includes a **Labels** submenu: a checklist
  of every ordinary label (check state from the right-clicked card via `Catalog::videoHasLabel`); toggling
  one drives the whole effective selection to that state via `Catalog::addLabel`/`removeLabel`, then
  `refreshLibraryView()`.
- `effectiveSelection(id)` — if `id` (a `VideoId`) is in the current multi-selection, returns every selected
...
- `refreshLibraryView()` — `refreshVideoGrid()` **then** `m_labelSidebar->refresh()` (reads label counts from
  the now-current model); the order matters, the sidebar would read stale counts if reversed. **Never call
  `m_labelSidebar->refresh()` from inside `refreshVideoGrid()` itself**: the sidebar's `itemClicked` emits
  `filterChanged` → `refreshVideoGrid`, so a refresh from in there would `clear()`+rebuild the sidebar's
  `QListWidget` from within its own click-signal chain — use-after-free on the item/view that's still on the
  call stack. This is why structural changes (add/delete/rename a video, create a label) call
  `refreshLibraryView`, while filter/sort/zoom changes call plain `refreshVideoGrid`. Direct consequence:
...
`effectiveSelection`:
- **Context-menu Labels checklist** — see `showVideoContextMenu` above.
- **Dragging a label row from `LabelSidebar` onto a card.** The sidebar is the drag source — its list
  ...
  drop target (see [video-widgets.md](video-widgets.md)). `addCard` wires each card's `setOnLabelDropped`
...
(never propagates), preserving the selection. `VideoItemWidget` forwards it; `MainWindow` wires it per-card to
...
through because `VideoItemWidget` now paints a **transparent** normal background (it owns only a rounded
...
1. Renames the source video file (if one is tracked and present) via `QFile::rename`.
...
3. `Catalog::applyRename(oldId, newId, newSourceVideoPath, newFolderAbs)` updates the model and the
   persisted record to match (see [catalog-and-labels.md](catalog-and-labels.md)). `VideoId` is filename +
   ...
   it, that metadata would be silently orphaned under the old `VideoId`. If the source file kept its name
   (only the frame folder moved by itself), `newId == oldId` and this is just a folder/path update.
   `applyRename` refuses (returns false) when the new id collides with another tracked video — the
...
`renameVideoInteractive(id)` is the UI-prompt wrapper (validate chars, check collisions, confirm) →
`renameVideo(...)`, reached from the card's context menu ("Rename video").

...
- **`applyRename(oldId, newId, newSourcePath, newFolderAbs)`** — the rename counterpart to `addMediaItem`:
  ...
  (`MediaRename`'s video/photo paths, see [main-window.md](main-window.md)) does
...
and inside label rename/delete workflows — **not** during browser filtering, sorting, zoom, or ordinary
refresh. The model is kept current by its own mutation API, so re-reading the store for presentation-only
changes would be redundant work. Item-level label mutations patch the affected entry in place instead of
triggering a reload.
